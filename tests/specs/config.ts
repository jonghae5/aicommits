...
export default testSuite(({ describe }) => {
	describe("config", async ({ test, describe }) => {
		...
		const configPath = path.join(fixture.path, ".dacommits");
		const openAiToken = "OPENAI_KEY=sk-abc";

		test("set unknown config file", async () => {
			const { stderr } = await dacommits(["config", "set", "UNKNOWN=1"], {
				reject: false,
			});
		...
		// 	const { stderr } = await dacommits(['config', 'set', 'OPENAI_KEY=abc'], {
		// 		reject: false,
		// 	});
		...
		await test("set config file", async () => {
			await dacommits(["config", "set", openAiToken]);
		...
		await test("get config file", async () => {
			const { stdout } = await dacommits(["config", "get", "OPENAI_KEY"]);
		...
		await test("reading unknown config", async () => {
			...
			const { stdout, stderr } = await dacommits(["config", "get", "UNKNOWN"], {
				reject: false,
			});
		...
		await describe("timeout", ({ test }) => {
			test("setting invalid timeout config", async () => {
				const { stderr } = await dacommits(["config", "set", "timeout=abc"], {
					reject: false,
				});
			...
			test("setting valid timeout config", async () => {
				...
				await dacommits(["config", "set", timeout]);

				const configFile = await fs.readFile(configPath, "utf8");
				expect(configFile).toMatch(timeout);

				const get = await dacommits(["config", "get", "timeout"]);
		...
		await describe("max-length", ({ test }) => {
			...
			test("must be at least 20 characters", async () => {
				const { stderr } = await dacommits(["config", "set", "max-length=10"], {
					reject: false,
				});
			...
			test("updates config", async () => {
				const defaultConfig = await dacommits(["config", "get", "max-length"]);
				expect(defaultConfig.stdout).toBe("max-length=50");

				const maxLength = "max-length=60";
				await dacommits(["config", "set", maxLength]);

				const configFile = await fs.readFile(configPath, "utf8");
				expect(configFile).toMatch(maxLength);

				const get = await dacommits(["config", "get", "max-length"]);
